...
def isTag(node, tagsuffix=''):
    return not isinstance(node, etree._Comment) and isinstance(node.tag, str) and node.tag.endswith(tagsuffix)
...
class ProcessDaveML:
    def __init__(self, filename):
        ...
        if self.splines:
            ...
            for var, spline in self.splines.items():
                ...
                knots_to_init_str = '[%s]' % ','.join([knot_var for knot_var in knot_vars])

                to_save_coeff = spline.coefficients.reshape(spline.coefficients.shape[:spline.xdim] + spline.yshape)
                new_line_str = ']'*(to_save_coeff.ndim-1) + ','
                coefficient = array_printer.doprint(
                    to_save_coeff
                ).replace(new_line_str , new_line_str+'\n')
                coeff_varname = '%s_coeffs' % (base_name)
                pre_lines.append('%s = %s' % (coeff_varname, coefficient))
                pre_lines.append('%s = ndsplines.NDSpline(%s, %s, 1)\n' % (base_name, knots_to_init_str, coeff_varname))

        pre_line_str = "\n".join(pre_lines)
        printer = codegen.ModulePrinter(MNPP)
        if self.input_vars:
            code = printer.codeprint([
                [funcname, sp.Array(self.input_vars), sp.Array(self.output_vars), self.extra_assignments]
            ], extra_pre_lines=pre_line_str)
        else:
            code = 'import numpy\n' + pre_line_str

        if self.check_data:
            check_data_code = "\ncheck_data = [\n [%s] \n]\n\n" % "],\n\n [".join(["%s,\n %s" % (list(check_input.values()), list(check_output.values())) for check_input, check_output in self.check_data])
    ...
    def check_dependencies(self, atoms):
        for symbol in atoms:
            if ((symbol not in self.input_vars)
