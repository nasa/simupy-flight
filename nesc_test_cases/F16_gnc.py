...
def F16_gnc(throttle, longStk, latStk, pedal, sasOn, apOn, circlePoleSW, ownshipN_deg, ownshipE_deg, keasCmd, altCmd, altMsl, Vequiv, alpha, beta, phi, theta, psi, pb, qb, rb, throttleTrim, longStkTrim):
    fsasOn = apOn + sasOn
    altErr = -altCmd + altMsl
    deltaThetaCmd = numpy.clip(altErr*thetaCmdAltErrGain,-5.0,5.0)
    thetaCmd = deltaThetaCmd + trimmedTheta
    deg_to_ft = 364567.2
    circleRadius_ft = 18228.36
    latOffsetPolar = -circleRadius_ft + deg_to_ft*(90.0 - ownshipN_deg)
    degEFromTgt = numpy.select([numpy.greater(ownshipE_deg, 0.0)], [ownshipE_deg - 180.0], default=ownshipE_deg + 180.0)
    ownshipN_ft = deg_to_ft*ownshipN_deg
    ownshipE_ft = degEFromTgt*deg_to_ft*numpy.cos(0.0174532925*ownshipN_deg)
    ftFromTgt = numpy.sqrt(ownshipE_ft**2 + ownshipN_ft**2)
    baseChiCmdEquatorIDL = -57.2957795785523*numpy.arctan2(ownshipN_ft, ownshipE_ft)
    latOffsetEquatorIDL = -circleRadius_ft + ftFromTgt
    latOffset = numpy.select([numpy.greater(circlePoleSW, 0.5)], [latOffsetPolar], default=latOffsetEquatorIDL)
    baseChiCmd = numpy.select([numpy.greater(circlePoleSW, 0.5)], [baseChiCmdPolar], default=baseChiCmdEquatorIDL)
    chiEst = beta + psi
