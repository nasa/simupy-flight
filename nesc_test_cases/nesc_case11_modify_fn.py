...
def get_ic_args_from_baseline(idx):
    ...
    try:
        long, lat, h = planet.planetodetics.pcf2pd(
            *baseline_df[["eiPosition_ft_X", "eiPosition_ft_Y", "eiPosition_ft_Z"]]
            / ft_per_m
        )
    except KeyError:
    ...
    try:
        ...
        pass
    try:
        flight_condition[7:10] = (
            baseline_df[["eiVelocity_ft_s_X", "eiVelocity_ft_s_Y", "eiVelocity_ft_s_Z"]]
            / ft_per_m
        )
    except KeyError:
...
def eval_trim(flight_condition, elevator, aileron, rudder, throttle):
    ...
    controller_func = get_controller_function(
        throttleTrim=throttle / 100,
        longStkTrim=elevator / -25.0,
        throttle=0.0,
        longStk=0.0,
        latStk=aileron,
        pedal=rudder,
    ...
    aero_plus_prop_acceleration = simupy_flight.dynamics.dynamics_output_function(
        F16_vehicle, 0, *kin_out, *controller_func(0, np.zeros(11))
    )

    gen_accel = aero_plus_prop_acceleration

    gen_accel[:3] = simupy_flight.kinematics.local_translational_trim_residual(
        planet, *flight_condition[:-3], *aero_plus_prop_acceleration[:-3]
    ).squeeze()
...
def run_trimmer(
    ...
    def trim_opt_func(x):
        ...
        return np.linalg.norm(
            weighting_matrix
            @ eval_trim(flight_condition, elevator, aileron, rudder, throttle),
            ord=2,
        )
    ...
    print(
        "pitch: %.4e  roll: %.4e  elevator: %.4f  aileron: %.4f  rudder: %.4f  throttle: %.4f"
        % (
            opt_theta * 180 / np.pi,
            opt_phi * 180 / np.pi,
            opt_elevator,
    ...
    print(
        "accelerations:\n",
        eval_trim(
            opt_flight_condition, opt_elevator, opt_aileron, opt_rudder, opt_throttle
        ).reshape((2, 3)),
    )
...
controller_block = systems.SystemFromCallable(
    get_controller_function(opt_ctrl[-1] / 100.0, opt_ctrl[0] / -25.0), dim_feedback, 4
)
